            ...
            */}
            <Card className="min-h-0 flex-1" innerClassName="flex h-full min-h-0 flex-col p-5">
              ...
              */}
              ...
              {/*
                **차트 높이는 flex 사슬이 아니라 자기 가로폭에서 나온다.** 이 자리는 원래
                `flex-1 min-h-0`으로 남는 세로 공간을 받았는데, 여기까지 오는 길에 `h-[100dvh]` →
                `flex-1 min-h-0` → `grid-rows-[minmax(0,1fr)]` → `h-full` → Card `flex-1` →
                `h-full min-h-0`이 예닐곱 겹 쌓여 있다. 크롬은 이 사슬을 끝까지 풀어 주지만
                **사파리는 못 풀어 차트가 높이 0에 가깝게 무너졌다** — 캔들이 얇은 띠로 눌리고
                거래량 막대와 오른쪽 가격축 라벨이 통째로 사라졌다(2026-08-22 맥 실사용 보고).

                그래서 같은 레포에서 이미 브라우저를 안 타는 패턴으로 바꿨다 — 튜토리얼 차트
                (`AttemptTutorialFlow`의 `aspect-[16/9] min-h-[240px]`)와 같은 방식이다. `aspect`가
                가로폭에서 높이를 정하므로 부모 사슬이 어떻든 높이가 확정되고, `min-h`가 폭이 좁을 때
                바닥을, `max-h-full`이 창이 낮을 때 넘침을 막는다.

                **`flex-1`을 다시 붙이지 않는다.** 붙이면 높이가 flex로 확정돼 `aspect`가 무시되고
                (크롬) 사파리와 해석이 갈린다 — 그 조합이 이번 버그의 뿌리였다.
              */}
              <div className="mt-2 flex aspect-[16/9] max-h-full min-h-[240px] flex-col">
                <CandleChart
                  candles={candles}
                  interval={interval}
