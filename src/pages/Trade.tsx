            ...
            */}
            <Card className="min-h-0 flex-1" innerClassName="flex h-full min-h-0 flex-col p-5">
              ...
              */}
              ...
              {/*
                차트는 flex-1 min-h-0 로 남는 세로 공간을 전부 가져간다. 창을 줄이면 스크롤 대신
                그대로 작아진다(2026-08-19 피드백). CandleChart 내부는 h-full(퍼센트 높이)이 아니라
                flex-1(그로우 기반)로 자기 몫을 채운다 — min-height 로 세운 flex 아이템 여러 겹을
                거치는 퍼센트 높이는 크롬이 "정의된 높이"로 인정하지 않아 0으로 무너지는 버그가
                있었다(2026-08-19 피드백 후속 수정 2).

                **여기에 aspect-[…]를 다시 붙이지 않는다.** 세로 flex 컨테이너에서 `flex: 1 1 0%`가
                높이를 확정하므로 크롬은 aspect-ratio를 **무시하고**, 사파리는 살려 쓴다. 그래서
                21:9 → 16:9 변경(2026-08-21)이 윈도우에서는 아무 일도 하지 않고 맥에서만 차트가
                78% 높아져 레이아웃이 깨졌다(2026-08-22 실사용 보고). 브라우저로 직접 재 보니 크롬에서
                두 비율의 렌더 결과가 픽셀 단위로 같았다 — 즉 이 자리의 비율 지정은 크롬 기준으로는
                처음부터 죽은 값이었다. 비율을 정말 고정해야 하면 flex-1을 떼고 튜토리얼 차트처럼
                (AttemptTutorialFlow의 aspect-[16/9] min-h-[240px]) 높이를 비율이 정하게 해야 한다.
              */}
