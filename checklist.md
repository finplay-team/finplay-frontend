...
## D. 시장가/지정가/프리셋을 게이트형 3단계로 분리 (피드백 2번, 이후 재확인) — spec 필요, 이번 범위 밖
사용자가 원하는 최종 구조: 2-1 시장가 체험 → (완료해야 열림) → 2-2 지정가 체험 → (완료해야 열림) →
3단계 손절·익절 프리셋 체험. "진짜로 단계를 잠그는 구조로" 가기로 확정(2026-08-20).
- [ ] 백엔드 planner 세션에서 spec 작성 — 아래 열린 질문에 먼저 답해야 한다
  - "완료"의 정의 — 시장가로 사기만 하면 되는지, 팔아서 그 진입을 정리해야 다음이 열리는지
  - 이 구조면 한 run 안에 최소 2~3번의 매수·매도 사이클이 필요하다. 042의 재진입
    (`entrySequence`)으로 기술적으로는 가능하지만, 지금 프론트(`AttemptTutorialFlow.tsx`)는
    한 사이클만 지원한다 — 화면도 다시 짜야 한다
  - 041 대본(사건·가격)은 run 전체에 걸친 고정 이야기라, 사이클을 3번 반복하면 3번째 진입은 이미
    상당히 진행된 구간에서 시작한다. "지정가 체험"·"프리셋 체험"이라는 학습 목적과 그 시점의
    이야기 전개가 자연스럽게 맞는지는 제품 쪽 검토가 필요하다
  - `PracticeEntryResponse`에 그 진입이 시장가였는지 지정가였는지가 없다(`orderType` 필드 없음) —
    서버가 게이트를 판정하려면 이 정보가 있어야 한다
  - **잠금은 서버가 판정해야 한다.** 프론트가 로컬로 판단하면 새로고침 한 번에 뚫리고 서버·화면의
    단계 인식이 어긋난다 — `steps[].locked` 같은 필드가 새로 필요하다
- [ ] spec이 나오면 프론트 연결 — 프리셋 픽커·선택 API는 이미 구현돼 있어(9차 A) 재사용 가능

## E. 코인 매수 폼을 실전(pages/Trade.tsx)과 맞추기 (피드백 2번 부속)
- [x] 라벨 "주문 금액", placeholder "최소 금액 ~원", "예상 매수"(수량+심볼) 문구를 실전과 동일하게
- [ ] "주문 가능" 잔액 표시·10/25/50/75/최대 퍼센트 버튼 — **막혀 있음**, `docs/backend-issues.md`
      E-1 참고(튜토리얼 계좌 잔액이 종목 선택 이후 0으로 옴)
...
## G. 백엔드에 등록할 것
- [ ] `docs/backend-issues.md` E-1 — 튜토리얼 계좌 잔액이 종목 선택 이후 0으로 온다
- [ ] `docs/backend-issues.md` E-2 — `practice_attempts` 잠금이 재시작·tick 동시 진행에서
      데드락(관찰됨 8~32회, 근본 원인은 백엔드에서 코드로 확인 필요)
- [ ] 위 D — 게이트형 3단계 구조 spec 착수 요청
