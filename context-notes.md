...
# 손절·익절 프리셋 → 자유 입력 재설계 (2026-08-21, 제안 A)

## D49. 프리셋 픽커를 없애고 실전 예약 매도 탭과 같은 자유 입력으로 바꿨다

제품 오너가 실사용 후 "실전 모의투자 화면과 방식이 달라 혼란스럽다"고 판단했다
(`docs/exit-plan-redesign-proposal-2026-08-21.md` 제안 A). 제안 B(손절·익절 각 1회 강제)와
C(체험 직후 반사실 모달)는 백엔드 spec 작성 중이라 이번 범위가 아니다.

**무엇을 본떴나.** 실전 `trade/OcoExitPlanPanel.tsx`의 예약 매도 탭 "비율(%)" 모드를 그대로
가져왔다 — 라벨 문구("손절 비율 (−%)"·"익절 비율 (+%)"), 오른쪽 정렬 tabular 입력, `%` 접미사,
손절은 `text-loss`·익절은 `text-gain` 포커스링, 아래에 붙는 기준가 한 줄까지 같다. 입력 정제
함수 `cleanDecimal`은 새로 쓰지 않고 실전 것에 `export`만 붙여 그대로 재사용한다 — 실전과
튜토리얼이 서로 다르게 입력을 받으면 여기서 익힌 조작이 실전에서 달라진다. 실전과 다른 점은
셋뿐이다. (1) 범위를 서버가 준 `exitRateBounds`로 막는다. (2) 폭의 뜻을 말로 돌려준다.
(3) "예약 걸기" 버튼이 없다 — 튜토리얼은 매수 체결 순간 서버가 자동으로 예약을 걸어 주므로
값이 바뀌면 곧바로 저장한다(프리셋을 누르면 바로 저장되던 감각을 유지한 것이다).

**프리셋 이름을 지운 자리를 무엇으로 메웠나.** 프리셋 3개는 단순한 입력 위젯이 아니라 교육
장치였다 — "조심스럽게/보통/느긋하게"라는 이름이 "손절 폭은 옳고 그름이 아니라 성향의 선택"
이라는 걸 가르쳤다. 이름표만 지우고 빈 숫자 칸을 남기면 그 의미가 통째로 사라진다. 되살릴 것은
이름이 아니라 의미라고 보고 두 가지를 넣었다.

1. **폭의 뜻을 한 줄로 돌려준다**(`ExitRateFields`). 범위 안에서의 **위치**(3등분)로 좁다/중간/
   넓다를 판정한다 — 경계 숫자로 판정하면 서버가 범위를 바꿨을 때 문구가 어긋난다. 어느 폭이
   옳다고 말하지 않고, 좁으면 좁은 대로 무엇을 얻고 무엇을 내주는지 양쪽을 함께 적는다.
2. **손절선·익절선을 차트에 미리 그린다**(`chartReferenceLines`). `CandleChart`가 이미
   `referenceLines`를 받으므로(원래 이 용도로 만들어져 있었다) 새로 만들 것이 없었다. 매수 전에는
   현재가로 잡은 어림선이라 라벨에 "예상"을 붙이고, 체결 후에는 종전처럼 서버가 확정한
   `stopLossPrice`·`takeProfitPrice`로 바꿔 그린다. **2단계(ORDER_BASICS)에는 그리지 않는다** —
   그 단계는 자동 청산 예약 자체를 만들지 않으므로(049 ORDERBASICS-022) 선을 그리면 없는 약속이
   된다. 입력 카드를 그 단계에서 안 그리는 것과 같은 이유다.

수량 기준 예상 손익 금액 병기(042 EXITPRESET-006의 취지)는 두 자리에서 유지된다 — 기존
`BuyRiskPreviewLine`이 이제 프리셋 대신 자유 입력 비율을 따르고, 입력칸 아래에도 수량이 적혀
있으면 그 수량 기준 금액을 함께 적는다.

## D50. 저장 시점 — 디바운스 자동 저장으로 정했다

프리셋은 누르는 순간이 곧 저장이었다. 자유 입력에는 그 순간이 없어 세 선택지가 있었다 —
(a) 별도 "저장" 버튼, (b) blur에 저장, (c) 입력이 멎으면 저장. (a)는 값을 고치고 저장을 안 누른 채
구매하면 **화면과 서버가 다른 값을 갖는** 조용한 어긋남이 생긴다. (b)는 구매 버튼 클릭이 blur를
먼저 일으켜 저장과 매수가 겹친다. (c)를 골랐다 — 500ms 멈추면 저장하고, 두 값이 **모두** 유효할
때만 보낸다(서버가 한 요청으로 둘을 함께 받으므로 한쪽이 비어 있는 동안 보내면 멀쩡한 다른
쪽까지 400이 된다).

**저장 중이라고 새 요청을 버리지 않는다.** 처음엔 프리셋 때처럼 `if (saving) return` 가드를
...
처음엔 서버가 `entry.exitPreset`에 무엇을 넣을지 몰라 손대지 않고 남겨 뒀는데, 백엔드 확정이
왔다 — **자유 비율이 옛 프리셋 3개와 정확히 일치할 때만 식별자가 오고 그 밖에는 `null`이다.**
즉 우려했던 빈 칩이 드물게가 아니라 **흔하게** 난다. 대신 `entries[]` 각 항목에 **그 진입에
실제로 적용된 손절률·익절률**이 진입별로 담긴다.

그래서 칩을 "조심스럽게" 같은 이름 대신 **`손절 −3% · 익절 +5%`**로 바꿨다. 사용자가 본 적 없는
말을 되돌려 주는 것보다 자기 손으로 넣은 숫자를 다시 보는 편이 학습 목적에 맞다. 부호는 입력
화면(`ExitRateFields`)과 같은 톤으로 붙인다(손절 −, 익절 +). **`exitPreset`은 이제 화면 어디에서도
쓰지 않는다** — 타입도 `PracticeExitPreset | null`로 정정했다.

**왜 attempt의 현재 값이 아니라 진입별 값인가.** 재진입 사이에 기준을 다시 정할 수 있으므로 한
실행 안에서도 진입마다 비율이 다를 수 있다. 완료 화면이 attempt의 현재 값을 그리면 1번째 진입
카드에 3번째 진입의 기준이 붙는다.
...
## D53. 백엔드 전수 조사 결과 — placeholder가 익절 하한을 권하고 있었다 (2026-08-21)

백엔드가 041 대본을 손절 [2,5] × 익절 [3,8] 0.1%p 간격 **1,581조합 전수**로 훑어 본 결과를
넘겨 줬다. 구간 안에서는 손절이 2막에서 100%, 익절이 3막에서 100% 도달하고, **1막에서 익절이
먼저 터지는 조합이 하나도 없다** — 즉 사용자가 어떤 값을 넣어도 손절·익절을 둘 다 겪고, 손절을
먼저 겪는 학습 순서가 구간 전체에서 보장된다. 범위 자체는 안전하다는 뜻이다.

**문제는 그 여유가 0.96%p뿐이라는 것이었다**(1막 고점 1.018 대 가장 좁은 익절선 1.02794).
익절이 대략 1.8% 아래로 내려가면 1막에서 익절이 먼저 터져 순서가 깨진다. 이 말을 듣고 내 코드를
다시 보니 **익절 입력창의 placeholder가 `bounds.takeProfitMin`, 즉 하한 3을 그대로 제안하고
있었다.** 서버가 3 미만을 400으로 막아 주니 "틀린 값이 저장되는" 문제는 아니지만, 화면이 사용자를
그 얇은 가장자리로 몰아 두는 신호였다. placeholder를 **지금 저장된 값**(`toRateInput(stopLossRate)`
·`toRateInput(takeProfitRate)`)으로 바꿨다 — 비웠을 때 원래 값을 되비추기만 하고 하한을 권하지
않는다. 회귀 테스트 1건으로 고정했고, `FALLBACK_EXIT_RATE_BOUNDS`와 정밀도 상수 주석에 "익절
하한을 임의로 낮추지 마라"는 근거를 남겼다.

**부수적으로 알게 된 것.** 손절 2.3~2.7% 구간은 진입 시점에 따라 루머(2막 초반)에서 털릴 수도,
2막 확정까지 버틸 수도 있다. 둘 다 정상적인 손절이지만 "같은 값을 넣은 두 사람이 다른 장면에서
털린다"는 뜻이다 — 지금 화면의 의미 문구는 폭의 성질만 말하고 특정 장면을 약속하지 않으므로
그대로 두면 된다. 장면을 특정하는 문구를 나중에 쓴다면 이 구간을 조심해야 한다.

**폴백 세 개는 배포 후 한 번에 뗄 수 있다.** 셋 다 서버가 항상 non-null로 채워 보내는 것이
확정됐고, 지금 옵셔널인 이유는 계약이 아니라 머지·배포 순서뿐이다.

- `attempt`의 두 rate — 미선택 실행에도 서버가 기본값 3·5를 채운다.
- 진입별 비율 — 042 이전에 만들어진 진입(세 컬럼이 다 빈 행)도 `exit_preset` → 기본값 3·5 순으로
  해석해 채워 보내므로, 배포 후에는 `rateFromPrices` 역산이 **도달 불가 경로**가 된다.
  `riskSnapshot`의 같은 두 필드도 동일하다.
- `exitRateBounds` — 루트와 `attempt` 양쪽 모두, attempt가 아예 없는 legacy 경로(즐겨찾기만·
  미착수·042 이전 chain)에서도 non-null이다. `FALLBACK_EXIT_RATE_BOUNDS`도 함께 뗄 대상이다.

**경계 넷 중 조여 있는 것은 익절 하한 하나뿐이라는 것도 함께 적어 뒀다.** 익절 상한은 15.8까지,
손절 상한은 13.1까지 올려도 여전히 닿는다 — 어느 쪽이 조여 있고 어느 쪽이 헐거운지 구분이 안 되면
다음 사람이 엉뚱한 경계를 조심하게 된다는 백엔드 지적을 그대로 반영했다. 백엔드도 같은 방어를
`TAKE_PROFIT_MIN` 상수 선언 위에 넣었다고 한다(근거는 전수 조사 테스트).

## D54. 예약 매도를 사용자에게 돌려줬다 — 자리를 옮긴 게 아니라 순서를 되돌린 것 (2026-08-21)

제품 오너 지적은 "매수는 지정가·시장가로 하고 **매도할 때 예약매도로 손절·익절을 겪는 것**인데,
지금 3단계는 매수할 때부터 예약매도가 떠 있다"였다. 맞는 지적이다. D49~D52에서 만든
`ExitRateFields`는 UI만 실전 예약 매도와 같았을 뿐 **자리가 매수 폼**이라, 사용자는 사기도 전에
청산 계획을 정하고 서버가 대신 걸어 줬다 — "예약 매도"라는 **행위 자체가 화면에서 사라져 있었다**.

그래서 이번엔 위젯을 옮긴 게 아니라 순서를 되돌렸다. 매수 폼은 실전 매수 폼과 같아졌고(시장가·
지정가·수량뿐), 보유가 생긴 뒤 매도 화면의 세 번째 탭에서 사용자가 직접 건다. 지금까지 그 탭이
항상 비활성이던 이유(매수 순간 서버가 자동으로 걸어서)가 이번에 사라졌다.

**실전 `OcoExitPlanPanel`은 재사용하지 않고 같은 모양으로 감쌌다.** 그 패널은 실계좌 `holding`과
`/api/exit-plans` 목록을 직접 읽는데 튜토리얼 종목은 샌드박스라 그 목록에 잡히지 않고(spec 033)
생성 경로도 전용 엔드포인트다. 실전 화면에 튜토리얼 분기를 심는 것보다 **입력 경험만 같게 하고
데이터 경로를 갈라놓는 쪽**이 낫다고 판단했다 — 라벨·설명·예약 카드 모양·버튼 문구는 실전과 맞춰
두어 여기서 익힌 조작이 실전에서 그대로 통한다. 취소만은 실전과 **같은 API**를 쓴다
(`DELETE /api/exit-plans/{id}`) — 예약 레코드 자체가 같은 것이라 전용 취소 경로를 만들 이유가 없다.

**"손절을 어디서 겪고 익절을 어디서 겪는지"를 화면이 말하게 한 것이 이번 작업의 절반이다.**
`ExitJourneyGuide`가 매수·매도 어느 쪽이든 같은 자리에 상시로 붙어, 사기 → 예약 걸기 → 기다리기
레일에서 지금 어느 칸인지와 국면별 다음 할 일을 말한다. 그리고 "손절 겪기·익절 겪기" 칩 두 개는
문구와 무관하게 **계속** 남아 이번 실행에서 무엇을 아직 못 겪었는지 보여준다. 손절만 겪었으면
"이제 익절을 겪어 볼 차례"라고 말하되 **잠그지는 않는다**(제품 오너가 확정한 강도 b) — 그만두고
되돌아보기로 마무리해도 된다는 말을 같은 문단에 함께 둔다.

**침묵하지 않기로 한 세 자리.** (1) 예약 없이 그냥 팔려는 순간 — 매도 버튼 아래에 "지금 팔면 직접
판 것이라 손절·익절은 겪지 않습니다"를 붙인다(버튼은 그대로 눌린다). (2) 예약을 취소한 뒤 —
"다시 걸지 않으면 값이 아무리 움직여도 저절로 정리되지 않는다"를 예약 폼과 안내 양쪽에 남긴다.
(3) 예약 없이 팔고 끝낸 뒤 — "직접 팔아서 정리했습니다. 손절·익절은 아직 겪지 않았어요."

**거짓말이 되는 문장 세 개를 함께 걷어냈다.** 자동 예약이 사라지면서 (a) 매수 폼의 "사는 순간
기준선 두 개가 자동으로 만들어집니다", (b) `RiskEducationCard`의 "이 선에 닿으면 자동으로
팔립니다", (c) 차트의 손절·익절 점선이 모두 **예약이 없으면 참이 아닌 문장**이 됐다. (a)는 삭제,
(b)·(c)는 예약이 있을 때만 그리도록 가드를 걸었다. 매수 전 손익 어림 줄(`BuyRiskPreviewLine`)도
같은 이유로 뺐다 — 같은 계산은 예약 폼 안에서 **실제로 걸 값과 보유 수량** 기준으로 한다.
기준가도 현재가에서 **평균 매수가**로 바꿨다. 서버가 예약선을 산 값에서 만들기 때문에, 현재가로
계산하면 화면이 말하는 선과 서버가 만드는 선이 어긋난다.

**아직 이름을 모르는 것.** 진행 조회에 추가된다는 두 덩어리(PENDING 예약 정보 / 손절·익절 경험
여부)의 필드 이름을 백엔드(`be-exit-plan-manual`)가 아직 안 줬다. 물어봤고 회신 대기 중이다.
가정한 이름은 `pendingExitPlan`·`stopLossExperienced`·`takeProfitExperienced`이고, **읽는 자리를
`services/tutorialExitPlan.ts`의 두 함수로 좁혀 뒀다** — 이름이 오면 그 파일과 타입 선언만 고치면
화면은 손대지 않는다.

경험 여부는 서버 필드가 없어도 **`entries[].sellCause`로 같은 판정을 만든다**(현재 실행 세대의
진입만 담기므로 질문이 정확히 일치한다). 그래서 배포 전에도 기능이 동작한다. 반면 PENDING 예약
정보는 대체할 소스가 없어, 방금 건 예약을 화면이 잠깐 들고 있는다(`estimateExitPlan`, 가격은
기준가에서 계산한 어림값이고 `exitPlanId`가 null이라 그 카드에는 취소 버튼이 뜨지 않는다).
서버 값이 오면 언제나 서버가 이긴다.

**폴백을 부분적으로 섞지 않는다.** 서버가 두 경험 필드 중 하나만 내려주는 어중간한 상태를 가정해
필드별로 섞으면 어느 판정이 어디서 왔는지 아무도 설명할 수 없는 값이 된다 — 하나라도 있으면 서버
판정만, 하나도 없으면 폴백만 쓴다.

**남은 확인거리.** (1) `POST .../exit-plan`의 거부 코드 문자열(보유 없음 / 이미 PENDING) — 지금은
서버 메시지를 그대로 보여준다. (2) 예약 생성이 `tutorialStageProgress.exitPresetSelected`를
참으로 만드는지 — 그 판정이 `PUT .../exit-rates` 전용이면 화면이 더 이상 그 경로를 부르지 않으므로
체크리스트 항목이 영영 안 채워진다. 그래서 화면은 "예약을 걸었거나 이미 겪었으면" 채운 것으로
그린다(서버가 판정을 넓히면 겹칠 뿐 어긋나지 않는다). (3) `PUT .../exit-rates`는 서버에 그대로
살아 있고 서비스 함수도 남겼다 — "매수 전에 기준을 미리 정해 둔다"는 용도로는 여전히 유효하지만
**매수 폼에서 노출하지는 않는다**.

## D55. 백엔드 계약 확정 — "예약은 진입당 한 번"이 화면 하나를 더 만들었다 (2026-08-21)

D54를 쓰고 나서 백엔드(`be-exit-plan-manual`) 회신이 왔다. 가정 대비 바뀐 건 두 boolean의 위치뿐이지만
(루트 → `exitExperience` 객체 안), **함께 온 사실 하나가 화면을 바꿨다.**

**예약은 한 진입에 한 번만 걸 수 있고, 취소해도 그 한 번이 소모된다**(write-once). 즉 "보유 중이고
예약이 없다"가 "예약을 걸 수 있다"와 같지 않다 — 걸었다 취소한 진입이 정확히 그 상태이고, 거기서 다시
걸면 409 `EXIT_PLAN_ALREADY_EXISTS`다. D54에서 나는 취소 뒤 "다시 걸지 않으면 손절·익절이 일어나지
않는다"고 안내하고 있었는데, **그 말대로 하면 누를 때마다 실패한다.** 그래서 서버가 함께 주는
`exitPlanCreatable`(POST가 201을 줄 조건과 같은 산출식)을 정본으로 삼고, 보유·예약 유무로 대신 계산하지
않는다. 그 값이 false면 국면이 `HOLDING_PLAN_SPENT`로 갈라져 "이 진입에는 다시 걸 수 없다 — 팔고 다시
사면 새로 걸 수 있다"고 말하고, 예약 탭을 열어도 눌리지 않는 폼 대신 그 이유가 뜬다. 공용
`EXIT_PLAN_ALREADY_EXISTS` 문구("취소한 뒤 다시 시도해 주세요")도 이 화면에서만 덮었다 — 실거래 OCO
에서는 맞는 말이라 공용 문구는 그대로 둔다.

**BigDecimal이 문자열로 올 수 있다.** 백엔드 예시 응답의 `stopLossPrice`가 `"9874.60000000"`이다.
이 레포의 `Decimal`은 `number` 별칭이라 타입은 통과하지만, 그대로 두면 `entryPrice - stopLossPrice`가
조용히 문자열 연결이 되어 금액 문구가 통째로 깨진다. `readPendingExitPlan()`에서 숫자로 강제하고
회귀 테스트로 고정했다(문자열 가격을 넣어도 "9,700원"·"약 600원"이 나오는지).

**그 뒤 백엔드 소스에서 직접 확인했다.** `PracticePendingExitPlanResponse`는 `BigDecimal` 필드이고
레포 어디에도 `WRITE_NUMBERS_AS_STRINGS` 설정이 없다 — 즉 실제로는 JSON **숫자**로 나가고, 회신의
문자열 예시는 정밀도를 보여 주려던 표기였다(백엔드도 이 오해를 확인하고 `docs/api/education.md`에
명시 + 컨트롤러 테스트에 `isNumber()` 단언으로 고정했다고 한다). 강제 변환은 무해하므로 방어로 남긴다(다른 튜토리얼
숫자 필드도 같은 규칙이라 이 한 곳만 방어해도 어긋나지 않는다).

**성공은 201이고 자연 멱등이 아니다.** 재시도해도 되는 요청으로 다루면 안 된다 — 화면은 실패 시
자동 재시도를 하지 않고 오류만 보여준다. 응답 본문(`ExitPlanResponse`)은 일부러 안 읽는다. 예약의
정본은 진행 조회이고, 본문 모양을 추측해 타입으로 굳히면 두 곳이 어긋난다.

**서버가 확인해 준 것 중 화면에 그대로 반영한 나머지.** `pendingExitPlan`은 예약이 없으면 필드 자체가
null(빈 껍데기 아님)이고 `entryPrice`·`quantity`·`reservedAt`이 더 온다 — 기준가는 이제 진입 기록에서
고른 값보다 **서버가 그 예약에 실어 준 `entryPrice`를 앞세운다**. 수동 매도는 어느 쪽 겪음으로도 세지
않는다(예약이 `FILLED_STOP_LOSS`/`FILLED_TAKE_PROFIT`로 체결된 것만) — 진입 기록 폴백이
`sellCause === 'MANUAL'`을 안 세는 것과 같아서 두 경로의 판정이 일치한다. `recommendedNext`도 받지만
지금은 안 읽는다. 겪지 않은 쪽을 화면이 이미 알고 있고(칩 두 개), 같은 판단을 두 소스에서 하면 어긋날
때 어느 쪽이 맞는지 설명할 수 없다.

**`exitPresetSelected`는 예약 생성으로 채워지지 않았다 — 그래서 서버가 넓혔다.**
`PracticeStageProgressCalculationService`가 이 값을 `attempt.exitRatesSelected()`로만 계산하고 예약
생성 서비스는 attempt의 그 컬럼을 건드리지 않는다는 걸 소스에서 확인해 물었더니, 오케스트레이터
결정으로 **서버 판정을 넓혔다는 회신이 왔다**(2026-08-21) — 이제 "`exit_stop_loss_rate`/`exit_preset`이
설정됨, **또는** 이 실행 세대에 예약이 하나라도 있음"이다. **상태 무관**이라 체결·취소된 예약도 세고,
취소해도 false로 돌아가지 않으며, 재시작하면 초기화된다(대본을 쓰지 않는 legacy 실행의 자동 예약만
제외 — 3단계 화면에는 해당 없다).

그래서 화면 OR(`exitPlanned`)는 **판정이 아니라 배포 전 폴백으로 성격이 바뀌었다.** 백엔드 지적대로
판정 소스는 하나가 낫고, **서버 값이 화면 판정보다 좁아지는 경우는 없다** — "겪었다"는 그 예약이
체결됐다는 뜻이라 서버 쪽이 항상 함께 참이기 때문이다. 다만 지금 걷어내면 **배포 전까지 그 체크
항목이 예약을 걸어도 안 채워지는 회귀**가 되므로, 다른 폴백들과 같은 취급으로 남기고 제거 대상
목록(checklist §14)에 넣었다. 배포되면 `exitPlanned` prop과 인자를 지우고
`progress.exitPresetSelected`만 그리면 된다.

**아직 못 한 것.** 백엔드가 커밋·배포 전이라 네트워크 검증은 여전히 못 했다. 로컬 8080은 인증 앞단에서
401을 돌려줘 라우트 존재 여부조차 확인이 안 된다(없는 경로도 같은 401이다).
