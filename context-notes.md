...
**진입별 비율 폴백은 배포 후 확실히 뗄 수 있다.** `PracticeEntryResponse.stopLossRate`·
`takeProfitRate`는 042 이전에 만들어진 진입(세 컬럼이 다 빈 행)도 서버가 `exit_preset` → 기본값
3·5 순으로 해석해 채워 보내므로 **항상 non-null**이다. 즉 배포 후에는 `rateFromPrices` 역산이
도달 불가 경로가 된다. `riskSnapshot`의 같은 두 필드도 동일하다.
